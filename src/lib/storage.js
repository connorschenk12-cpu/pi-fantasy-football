...
export function listenTeam({ leagueId, username, onChange }) {
  const teamRef = doc(db, "leagues", leagueId, "teams", username);
  return onSnapshot(teamRef, (snap) => onChange(snap.exists() ? snap.data() : null));
}
...
/* =========================================
   DRAFT: legacy claim + turn-based snake draft
   ========================================= */
...
// (Legacy simple claim – still handy for testing)
...
// Turn-enforced snake draft; records pick & advances pointer/round/direction.
...
export async function initLeagueDefaults(leagueId) {
  ...
  await updateDoc(ref, { draft: defaults.draft, settings: defaults.settings });
...
export async function scheduleDraft(leagueId, isoDateString) {
  ...
  const ref = doc(db, "leagues", leagueId);
  await updateDoc(ref, {
    draft: { ...(await (await getDoc(ref)).data()?.draft), status: "scheduled", scheduledAt: when.toISOString(), startedAt: null },
  });
}
...
export async function setDraftStatus(leagueId, status) {
  ...
  const snap = await getDoc(ref);
  const data = snap.data() || {};
  const prev = data.draft || {};
  const patch =
    status === "live"
      ? { draft: { ...prev, status: "live", scheduledAt: null, startedAt: new Date().toISOString() } }
      : { draft: { ...prev, status } };
