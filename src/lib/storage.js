...
export const BENCH_SIZE = 3; // 9 starters + 3 bench = 12 total rounds
...
export async function initDraftOrder({ leagueId }) {
  ...
  if (!members.length) throw new Error("No members to seed draft order.");
...
/** Auto-create season schedule if missing. */
export async function ensureSeasonSchedule({ leagueId }) {
  // If week-1 exists, assume schedule already written
  const w1 = await getDoc(doc(db, "leagues", leagueId, "schedule", "week-1"));
  if (w1.exists()) return;

  const members = await listMemberUsernames(leagueId);
  if (members.length < 2) return;

  // Single round-robin up to 14 weeks, but never more than teams-1
  const totalWeeks = Math.min(14, Math.max(1, members.length - 1));
...
export async function endDraft({ leagueId }) {
  ...
  // Auto-create schedule once draft completes
  await ensureSeasonSchedule({ leagueId });
}
...
export async function setDraftStatus({ leagueId, status }) {
  ...
  if (status === "done") await ensureSeasonSchedule({ leagueId });
}
...
export async function draftPick({ leagueId, username, playerId, playerPosition, slot }) {
  ...
  if ((await getDoc(claimRef)).exists()) throw new Error("Player already owned");
  ...
  const teamsCount = Math.max(1, order.length);
  ...
  // If that was the final pick, ensure schedule exists
  if (doneAll) await ensureSeasonSchedule({ leagueId });
}
...
export async function listMyLeagues({ username }) {
  ...
  const sOwned = await getDocs(query(leaguesCol, where("owner", "==", username)));
...
/** Single round-robin, up to totalWeeks */
...
export async function listSeasonSchedule(leagueId) {
  const col = collection(db, "leagues", leagueId, "schedule");
  const snap = await getDocs(query(col, orderBy("week", "asc")));
  const arr = [];
  snap.forEach((d) => arr.push({ id: d.id, ...d.data() }));
  // ensure numeric week sort fallback
