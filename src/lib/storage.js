...
export function emptyRoster() {
  const r = {}; ROSTER_SLOTS.forEach((s)=>r[s]=null); return r;
}
...
/* =========================================================
 * MEMBERS / ENTRY / PAYMENTS (client-side demo)
 * ======================================================= */
...
export function hasPaidEntry(league, username){
  ...
  if(!en) return true;
  return !!league?.entry?.paid?.[username];
}
export async function setEntrySettings({ leagueId, enabled, pricePi }){
  await updateDoc(doc(db,"leagues",leagueId),{
    entry:{ enabled: !!enabled, pricePi: Number(pricePi||0), paid:{} }
  });
}
export async function markEntryPaid({ leagueId, username, txId=null, amountPi=0 }){
  const ref = doc(db,"leagues",leagueId);
  const snap = await getDoc(ref); if(!snap.exists()) throw new Error("League not found");
  const paid = { ...(snap.data()?.entry?.paid || {}) };
  paid[username] = { at: Date.now(), txId: txId||"demo", amountPi: Number(amountPi||0) };
  await updateDoc(ref,{ "entry.paid": paid });
}
export async function allMembersPaid(leagueId){
  const league = await getLeague(leagueId); if(!league) return false;
  if(!league.entry?.enabled) return true;
  const members = await listMembers(leagueId);
  return members.length>0 && members.every(u=>!!league.entry?.paid?.[u]);
}

/* =========================================================
 * PLAYERS / PROJECTIONS / OPPONENT
 * ======================================================= */
export async function listPlayers({ leagueId }){
  if(leagueId){
    const lref = collection(db,"leagues",leagueId,"players");
    const lsnap = await getDocs(lref);
    if(!lsnap.empty){ const arr=[]; lsnap.forEach(d=>arr.push({id:d.id,...d.data()})); return arr; }
  }
...
export async function listPlayersMap({ leagueId }){
  ...
  const m=new Map(); arr.forEach(p=>m.set(p.id,p)); return m;
}
export function playerDisplay(p){ return p?.name || p?.fullName || p?.playerName || String(p?.id||"(empty)"); }
...
export function projForWeek(p, week){
  ...
  const keyed = p?.[`projW${week}`]; if(keyed!=null) return Number(keyed)||0;
  return 0;
}
...
export function pointsForPlayer(p, week){ return projForWeek(p, week); }
export function computeTeamPoints({ roster, week, playersMap }){
  const lines=[]; let total=0;
  (ROSTER_SLOTS||[]).forEach(slot=>{
    const pid = roster?.[slot]||null;
    const pl = pid ? playersMap.get(pid):null;
    const pts = pl ? pointsForPlayer(pl, week):0;
    total += Number(pts||0);
    lines.push({ slot, playerId: pid, player: pl, points: pts });
  });
...
export function computeSeasonProjected({ roster, playersMap, weeks=18 }){
  ...
  for(let w=1; w<=weeks; w++){
    (ROSTER_SLOTS||[]).forEach(slot=>{
      const pid=roster?.[slot]; const p = pid ? playersMap.get(pid):null;
      total += p ? projForWeek(p,w) : 0;
    });
  }
...
export function listenLeagueClaims(leagueId, onChange){
  ...
  return onSnapshot(ref,(snap)=>{
    const m=new Map(); snap.forEach(d=>m.set(d.id,d.data())); onChange(m);
  });
}
...
export function isMyTurn(league, username){
  const d = league?.draft||{}; const order=Array.isArray(d.order)?d.order:[];
...
export async function configureDraft({ leagueId, order }){
  ...
  await updateDoc(lref,{
    draft:{
      status:"scheduled", order:Array.isArray(order)&&order.length?order:prev?.draft?.order||[],
      pointer:0, direction:1, round:1, picksTaken:0, roundsTotal:DRAFT_ROUNDS_TOTAL,
      clockMs:PICK_CLOCK_MS, deadline:null,
    },
...
export async function startDraft({ leagueId }){
  // block until everyone has paid (if enabled)
  const league = await getLeague(leagueId);
  if (league?.entry?.enabled) {
    const ok = await allMembersPaid(leagueId);
    if (!ok) throw new Error("All members must pay entry fee before draft can start.");
  }
  await updateDoc(doc(db,"leagues",leagueId),{
    "draft.status":"live","draft.deadline": Date.now()+PICK_CLOCK_MS,
  });
}
export async function endDraft({ leagueId }){
  await updateDoc(doc(db,"leagues",leagueId),{
    "draft.status":"done","draft.deadline":null,"settings.lockAddDuringDraft":false,
  });
}
...
export async function autoPickBestAvailable({ leagueId, currentWeek }){
  ...
  const order = league?.draft?.order||[]; const ptr = Number(league?.draft?.pointer||0);
  const username = order[ptr]; if(!username) return;
...
export async function autoDraftIfExpired({ leagueId, currentWeek=1 }){
  const leagueRef = doc(db,"leagues",leagueId); const snap = await getDoc(leagueRef);
  if(!snap.exists()) return { acted:false, reason:"no-league" };
  const league = snap.data(); if(!canDraft(league)) return { acted:false, reason:"not-live" };

  const now=Date.now(); const clockMs=Number(league?.draft?.clockMs||PICK_CLOCK_MS);
  ...
  const post = (await getDoc(leagueRef)).data();
...
export async function moveToStarter({ leagueId, username, playerId, slot }){
  ...
  const s = await getDoc(tRef); if(!s.exists()) throw new Error("Team not found");
  const team = s.data();
...
export async function moveToBench({ leagueId, username, slot }){
  const tRef = doc(db,"leagues",leagueId,"teams",username);
  const s = await getDoc(tRef); if(!s.exists()) throw new Error("Team not found");
  const team=s.data(); const roster={ ...(team.roster||emptyRoster()) };
  const bench=Array.isArray(team.bench)?[...team.bench]:[];
...
export async function releasePlayerAndClearSlot({ leagueId, username, playerId }){
  ...
  const team=s.data(); const roster={ ...(team.roster||emptyRoster()) };
  const bench=Array.isArray(team.bench)?[...team.bench]:[];
  Object.keys(roster).forEach(k=>{ if(roster[k]===playerId) roster[k]=null; });
  const i = bench.indexOf(playerId); if(i>=0) bench.splice(i,1);
  const batch = writeBatch(db);
...
export async function addDropPlayer({ leagueId, username, addId, dropId }){
  ...
  if(dropId){
    ...
    Object.keys(roster).forEach(k=>{ if(roster[k]===dropId) roster[k]=null; });
...
export async function createLeague({ name, owner, order }){
  const ref = await addDoc(collection(db,"leagues"),{
    ...
    entry:{ enabled:false, pricePi:0, paid:{} },
    draft:{
      status:"scheduled",
...
export async function listMyLeagues({ username }){
  ...
  for(const d of all.docs){
    ...
    if(memSnap.exists() && !out.find(x=>x.id===d.id)) out.push({id:d.id,...d.data()});
  }
  return out;
}

/* =========================================================
 * SCHEDULE (auto-generate + read)
 * ======================================================= */
export function generateScheduleRoundRobin(usernames, totalWeeks){
  ...
  for(let week=1; week<=rounds; week++){
    ...
    for(let i=0;i<half;i++){
      const home=left[i], away=right[i];
      if(home!=="__BYE__" && away!=="__BYE__") matchups.push({ home, away });
    }
    schedule.push({ week, matchups });
    // rotate (circle method)
    const fixed = left[0];
    const movedLeftTail = left[left.length-1];
    left = [fixed, ...right.slice(0,1), ...left.slice(1, left.length-1)];
    right = [...right.slice(1), movedLeftTail].reverse().reverse(); // keep simple
  }
  return schedule;
}
...
export async function ensureOrRecreateSchedule({ leagueId, totalWeeks=14 }){
  ...
  const sched = generateScheduleRoundRobin(members, totalWeeks);
  await writeSchedule(leagueId, sched);
  return sched.length;
}
export async function getScheduleWeek(leagueId, week){
  const s = await getDoc(doc(db,"leagues",leagueId,"schedule",`week-${week}`));
