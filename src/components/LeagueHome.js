...
export default function LeagueHome({ league, me, onBack }) {
  ...
  return (
    <div style={{ marginTop: 8 }}>
      ...
      <h2>{l.name}</h2>
      <p><strong>League ID:</strong> <code>{l.id}</code></p>
      ...
      <div style={{ display: "flex", gap: 8, marginTop: 16, flexWrap: "wrap" }}>
        <button onClick={() => setTab("team")} style={{ padding: 8, fontWeight: tab === "team" ? 700 : 400 }}>
          My Team
        </button>
        <button onClick={() => setTab("players")} style={{ padding: 8, fontWeight: tab === "players" ? 700 : 400 }}>
          Players
        </button>
        <button onClick={() => setTab("draft")} style={{ padding: 8, fontWeight: tab === "draft" ? 700 : 400 }}>
          Draft
        </button>
        {isOwner && (
          <button onClick={() => setTab("admin")} style={{ padding: 8, fontWeight: tab === "admin" ? 700 : 400 }}>
            Admin
          </button>
        )}
      </div>
      ...
      <div style={{ marginTop: 16 }}>
        {tab === "team" && <MyTeam leagueId={l.id} username={me} onBack={() => setTab("players")} />}
        {tab === "players" && <PlayersList leagueId={l.id} username={me} />}
        {tab === "draft" && <DraftBoard leagueId={l.id} username={me} />}
        {tab === "admin" && isOwner && <LeagueAdmin leagueId={l.id} me={me} owner={l.owner} />}
      </div>
    </div>
  );
}
